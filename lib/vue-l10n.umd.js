...
function extend(Vue) {
   ...
   * 美元转当地货币金额
   * @param {*} usdAmount 
   * @param {*} type _computeTypeEnum 局部配置>全局配置
   * @returns 
   */
  ...
  Vue.prototype.$uts = function (usdAmount, type) {
    ...
    var computeType = type || l10nCurrency.currency.type;
...
function () {
  function VueL10nCurrency(options) {
    ...
    var locales = options.locales || 'en-US';
  ...
  _createClass(VueL10nCurrency, [{
    ...
    key: "_formatAmount",
    value: function _formatAmount(amount, computeType) {
      ...
      var notRoundingType = [this._computeTypeEnum.ORIGINAL, this._computeTypeEnum.INT, this._computeTypeEnum.INT_CARRY, this._computeTypeEnum.INT_TRUNCATION, this._computeTypeEnum.INT_RT];
      ...
      switch (computeType) {
        ...
        case this._computeTypeEnum.ROUNDING:
          amount = new big_default.a(amount);
      ...
      var formatResult = computeResult;
      ...
      if (computeType != this._computeTypeEnum.ORIGINAL) {
        ...
        var intTypeArr = [this._computeTypeEnum.INT, this._computeTypeEnum.INT_CARRY, this._computeTypeEnum.INT_TRUNCATION, this._computeTypeEnum.INT_RT];
        var decimalsTypeArr = [this._computeTypeEnum.ROUNDING, this._computeTypeEnum.CARRY, this._computeTypeEnum.TRUNCATION];
        ...
        var newLocales = "".concat(this.currency.locales, "-u-nu-latn");
        formatResult = new Intl.NumberFormat(newLocales, options).format(computeResult);
      }

      formatResult = formatResult.toString(); // 部分货币符号展示结果映射
    ...
    key: "_uts",
    value: function _uts(usdAmount, computeType, usdToSelfExchangeRate) {
      // console.log('[vue-l10n-currency] _uts.')
      // usdAmount 传进来的金额的可能是空 usdAmount = ''
      usdAmount = usdAmount ? new big_default.a(usdAmount) : usdAmount;
      usdToSelfExchangeRate = new big_default.a(usdToSelfExchangeRate);
      var selfAmount;
    ...
    key: "_stu",
    value: function _stu(selfAmount, computeType, SelfToUsdExchangeRate) {
    ...
    key: "_textUts",
    value: function _textUts(usdText, computeType, usdToSelfExchangeRate) {
...
src_VueL10nCurrency.version = '1.1.3';
